...
# Shared installation script for Debian-based systems (Ubuntu, Debian, etc.)
# This script works for any system using apt package manager

# Detect the specific distribution name
...
echo "Installing dependencies for $DISTRO_NAME..."
...
if ! command -v node >/dev/null 2>&1; then
    ...
    if curl -fsSL https://deb.nodesource.com/setup_lts.x | sudo -E bash - && sudo apt-get install -y nodejs; then
        ...
        echo "⚠️  NodeSource installation failed. Trying $DISTRO_NAME repository..."
        # Fallback to distribution's nodejs package
        if sudo apt-get update && sudo apt-get install -y nodejs npm; then
            echo "✓ Node.js installed via $DISTRO_NAME repository"
        else
